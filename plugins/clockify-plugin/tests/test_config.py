from clockify_plugin.config import Defaults, load_config, load_defaults
...
def test_load_config_le_do_arquivo(monkeypatch, tmp_path):
    ...
    from clockify_plugin.config import write_raw
...
def test_config_path_respeita_xdg(monkeypatch, tmp_path):
    monkeypatch.setenv("XDG_CONFIG_HOME", str(tmp_path))
    from clockify_plugin.config import config_path

    assert config_path() == tmp_path / "clockify-plugin" / "config.json"
...
def test_write_raw_cria_arquivo(monkeypatch, tmp_path):
    ...
    from clockify_plugin.config import config_path, read_raw, write_raw
...
def test_read_raw_ausente_retorna_vazio(monkeypatch, tmp_path):
    monkeypatch.setenv("XDG_CONFIG_HOME", str(tmp_path))
    from clockify_plugin.config import read_raw
...
def test_load_defaults_sem_secao_tolerante(monkeypatch, tmp_path):
    monkeypatch.setenv("XDG_CONFIG_HOME", str(tmp_path))
    from clockify_plugin.config import load_defaults, write_raw
