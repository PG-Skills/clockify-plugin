...
def test_learned_path_respeita_xdg(monkeypatch, tmp_path):
    ...
    from clockify_plugin.learned import learned_path

    assert learned_path() == tmp_path / "clockify-plugin" / "learned.json"


def test_record_e_list_roundtrip(monkeypatch, tmp_path):
    monkeypatch.setenv("XDG_CONFIG_HOME", str(tmp_path))
    from clockify_plugin.learned import learned_path, read_learned, record
...
def test_record_dedup_por_match(monkeypatch, tmp_path):
    monkeypatch.setenv("XDG_CONFIG_HOME", str(tmp_path))
    from clockify_plugin.learned import read_learned, record
...
def test_read_learned_ausente_vazio(monkeypatch, tmp_path):
    monkeypatch.setenv("XDG_CONFIG_HOME", str(tmp_path))
    from clockify_plugin.learned import read_learned
...
def test_read_learned_corrompido_vazio(monkeypatch, tmp_path):
    monkeypatch.setenv("XDG_CONFIG_HOME", str(tmp_path))
    from clockify_plugin.learned import learned_path, read_learned
